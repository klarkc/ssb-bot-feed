...
import {striptags, pull, toPull, request} from '../deps.js'
...
export default config => (err, sbot) => {
    ...
    const onPost = (entry) => {
        ...
        const renderPost = (image = '', mentions) => {
            ...
            const text = postTemplate.replace(
                /{(title|image|description|link)}/gm,
                match => matchMap[match.replace(/({|})/g, '')]
            );
            sbot.publish(
                { type: 'post', text: text.substr(0, 8192), mentions },
